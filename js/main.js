...
const anime = BASE_URL + "/discover/movie?sort_by=popularity.des&page=50&" + API_key;
...
function showmovies(data, movie_type) {
  data.forEach((movie)=> {
    const { title, poster_path, vote_average, release_date, genre_ids, overview, vote_count, popularity } = movie;
    ...
    movieCard.onclick = function() {
      document.getElementById("des").innerHTML=`<div class='modal-bg modal-active'>
      <div class='modal-main'>
        <span class='modal-btn-close' id='${title}'>X</span>
        <div class="movie-infoa">
         <div class="movie-name">Movie name : ${title}</div>
         <div class="movie-year">release date : ${release_date}</div>
         <div class="popularity">Popularity : ${popularity}</div>
         <div class="vote_count">Total vote : ${vote_count}</div>
        <div class="movie-rating">
